...
`--loop-resume` surface this condition - so by the time the poll appears the work is already halted and
...
`--loop-resume` SLEEP directive prints this same instruction when it detects the condition
(`multi_hop and 5h >= 99`).
